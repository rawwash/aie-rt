...
/*****************************************************************************/
/**
* @file xaie_partition_list_query.c
* @{
*
* This file contains the test application of dma loopback for aie.
*
* This application will be used to get the existing partition details from the
* kernel and will prints the partition details and user can able to select one
* partition by giving the PartitionId nd returns corresponding partition fd to
* facilitating debugging and perfomance measurement. This support is added for
* linux backend only and application writes random integer values to AIE
* data memory at location DATA_MEM_INPUT_ADDR on Tile_1. The DMA(MM2S channel)
* on Tile_1 is configured to read data from DATA_MEM_INPUT_ADDR and push it to
* stream switch DMA port. This data is received by stream switch on Tile_2.
* The DMA(S2MM channel) on Tile_2 configured to push data from stream switch to
* data memory buffer(DATA_MEM_INPUT_ADDR).
...
* ----- ------  -------- -----------------------------------------------------
* 1.0   jaydip   22/05/2024  Initial creation
* </pre>
*
******************************************************************************/
...
int main()
{
	...
	RC = XAie_GetPartitionFdList(&DevInst);
	if (RC != XAIE_OK) {
		printf("XAie_GetPartitionFdList failed \n");
		return -1;
	}

	printf("Select the partition id from list: \n");
	scanf("%u", &PartitionId);

	AiePartfd = XAie_SelectPartitionFromList(&DevInst, PartitionId);
	if (AiePartfd < 0) {
		printf("XAie_SelectPartitionFromList failed\n");
		return -1;
	}

        RC = XAie_SetupPartitionConfig(&DevInst, XAIE_BASE_ADDR,
        ...
        ConfigPtr.PartProp.Handle = AiePartfd;
	...
	RC = XAie_DestroyPartitionList(&DevInst);
	if (RC != XAIE_OK) {
		printf("XAie_DestroyPartitionList failed.\n");
		return -1;
	}
